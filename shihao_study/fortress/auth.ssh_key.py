...
def add_user_key(ip, user):
    for i in user.split(','):
        ...
        if "failed" in tmp_key:
            ...
            if tmp1[0] == 0:
                if role != "root":
                    chk_dir = 'ssh %s -o ConnectTimeout=10 "[ -d /home/%s/.ssh ] || mkdir -p /home/%s/.ssh && chown -R %s:%s /home/%s/.ssh/"' % (
                    ip, role, role, role, role, role)
                    commands.getstatusoutput(chk_dir)
                    chk_file = 'ssh %s -o ConnectTimeout=10 "[ -f /home/%s/.ssh/authorized_keys ] || touch /home/%s/.ssh/authorized_keys && chmod 600 /home/%s/.ssh/authorized_keys && chown -R %s:%s /home/%s/.ssh/*"' % (
                    ip, role, role, role, role, role, role)
