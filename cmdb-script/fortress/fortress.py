...
def fortress_help():
    ...
    5.refresh host list
    0.exit
    env:input env(like:dev, test, uat, stg, prod) to filter host list
    Others:input ip address or hostname to connect to host
    if you want to connect to host by your account not default(root/deploy), add your account after host split by space,for example:10.1.2.48 account'''
