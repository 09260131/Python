...
#***************************************************************#
#example: python yan.py add/del 10.0.128.23,10.0.128.45 root/admin/rd chenshaodong
...
def add_user_key(ip,user):
    for i in user.split(','):
        ...
        if "failed" in tmp_key:
            ...
            if tmp1[0] == 0:
                if role != "root":
                    chk_dir = 'ssh %s -o ConnectTimeout=1 "[ -d /home/%s/.ssh ] || mkdir -p /home/%s/.ssh && chown -R %s:%s /home/%s/.ssh/"'  % (ip,role,role,role,role,role)
                    commands.getstatusoutput(chk_dir)
                    chk_file = 'ssh %s -o ConnectTimeout=1 "[ -f /home/%s/.ssh/authorized_keys ] || touch /home/%s/.ssh/authorized_keys && chmod 600 /home/%s/.ssh/authorized_keys && chown -R %s:%s /home/%s/.ssh/*"'  % (ip,role,role,role,role,role,role)
                    commands.getstatusoutput(chk_file)
                else:
                    chk_dir = 'ssh %s -o ConnectTimeout=1 "[ -d /root/.ssh ] || mkdir -p /root/.ssh && chown -R root:root /root/"'  %  ip
                    commands.getstatusoutput(chk_dir)
                    chk_file = 'ssh %s -o ConnectTimeout=1 "[ -f /root/.ssh/authorized_keys ] || touch /root/.ssh/authorized_keys && chmod 600 /root/.ssh/authorized_keys"'  % ip
                    commands.getstatusoutput(chk_file)
...
def del_user_key(ip,user):
    for i in user.split(','):
        ...
        if "failed" in tmp_key:
            ...
            if tmp[0] == 0:
                cmd_del_root = 'ssh %s -o ConnectTimeout=1 "sed -i \'/%s/d\' /root/.ssh/authorized_keys" &>/dev/null' % (ip,tmp_key.replace('/','\/'))
                cmd_del_admin = 'ssh %s -o ConnectTimeout=1 "sed -i \'/%s/d\' /home/admin/.ssh/authorized_keys" &>/dev/null' % (ip,tmp_key.replace('/','\/'))
                cmd_del_rd = 'ssh %s -o ConnectTimeout=1 "sed -i \'/%s/d\' /home/rd/.ssh/authorized_keys" &>/dev/null' % (ip,tmp_key.replace('/','\/'))
